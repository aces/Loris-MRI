...
```bash
bids_import -d /PATH/TO/BIDS/TO/IMPORT -p config.py
```
...
```bash
bids_import -d /PATH/TO/BIDS/TO/IMPORT -p config.py -c -s
```

Finally, the verbose option can be turned on by using the option `-v` when
calling the script:
```bash
bids_import -d /PATH/TO/BIDS/TO/IMPORT -p config.py -v
```
