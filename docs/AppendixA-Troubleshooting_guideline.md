...
|`Number of MINC files that will be considered for inserting into the database: 0. No data could be converted into valid MINC files. Your project excluded series descriptions will not be considered!`|The upload contains only acquisitions that have SeriesDescriptions which are excluded by the project|Projects can specify what acquisitions' SeriesDescription are not to be processed by default in LORIS|
|My uploaded DICOM study contains 6 modalities but only 5 were inserted. The 6th modality was not converted to MINC, neither inserted into the MRI Violated Scans module. In addition, during insertion, the message `Number of MINC files that will be considered for inserting into the database: 5` is displayed, confirming that only 5 files are considered|The missing modality is probably an acquisition excluded by the project. `tarchiveLoader` is specifically 'instructed' to exclude some modalities as specified in the Imaging Pipeline section of the Configuration module of LORIS|No action needed. This is the expected behavior of the LORIS-MRI insertion pipeline|
