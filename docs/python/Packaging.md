...
The LORIS Python installation process should take care of installing LORIS Python on the host machine without the user needing to know the intricacies of Python or LORIS Python packaging. As such, this documentation is aimed at developers or power users that want to have a more granular control or better understanding of the LORIS Python packaging process.
...
pip install /path/to/local/package   # From a local directory
```
...
- Editable (option `-e` or `--editable`) that instead uses a symbolic link to the original source code.
...
Examples: `loris-utils`, `loris-bids-utils`
...
- Individual LORIS Python packages may increment their _minor_ version, in which case dependent packages should then also be updated.
...
Before LORIS Python used proper Python packaging, it made its Python code available to the environment through the `PYTHONPATH` environment variable, which is still used by some projects.
...
pip uninstall .
# Step 4: Add the LORIS Python packages directory (which notably contains the `lib` module) to the
#`PYTHONPATH` variable.
