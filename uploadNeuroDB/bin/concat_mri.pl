...
=item *
B<-clean> : remove the original files when they are concatenated

=item *
B<-clobber> : overwrite existing output file(s)

=item *
B<-debug> : print commands but do not execute them

=item *
B<-verbose> : print additional information while running
...
=item *
B<-ignorecontrast> : ignore the contrast agent flag in the file headers

=item *
B<-minslicesep sep> : do not concatenate the files if the resulting slice separation 
is less than C<sep>. If this option is not used, C<-minslicesep 1> is assumed.

=item *
B<-maxslicesep sep> : do not concatenate if the resulting slice separation is more 
than C<sep>. If this option is not used, C<-maxslicesep 3> is assumed.

=item *
B<-nonslicetolerance tolerance> : resample input volumes when the difference in 
start values in one of the non-slice dimensions exceeds C<tolerance>. If this option
...
=item *
B<-step step> : step used in final resampling. By default, the step size is computed
automatically.
