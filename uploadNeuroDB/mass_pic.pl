...
my $Usage = "mass_pic.pl generates check pic images for NeuroDB for those files that are missing pics.
...
my $sth = $dbh->prepare("SELECT f.FileID FROM files AS f LEFT OUTER JOIN check_pic_filenames AS c USING (FileID) WHERE c.FileID IS NULL AND f.FileType='mnc' $extraWhere");
$sth->execute();
