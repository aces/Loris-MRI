...
def get_meg_sensors(env: Env, physio_file: DbPhysioFile) -> MegSensorsResponse:
    ...
    for channel in raw.info["chs"]:  # type: ignore
        ...
        sensors[channel['ch_name']] = MegSensorPoint(
            x = finite_float_or_none(head_pos[0]),
            y = finite_float_or_none(head_pos[1]),
            z = finite_float_or_none(head_pos[2]),
            unit = get_ephys_unit_symbol(channel['unit']),  # type: ignore
...
def finite_float_or_none(value: float) -> float | None:
    """Convert a numeric coordinate to a finite JSON number, or null if it is not finite."""
