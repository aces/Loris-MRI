...
def import_bids_meg_ctf_data_type(
    env: Env,
    ...
    """
    Import a BIDS MEG CTF data type directory into LORIS.
    """

    if data_type.head_shape_file is not None:
        head_shape_file = import_bids_meg_ctf_head_shape_file(
            env,
            importer,
            session,
            data_type.name,
            data_type.head_shape_file,
       )
    else:
    ...
    import_bids_acquisitions(
        ...
        lambda acquisition, bids_info: import_bids_meg_ctf_acquisition(
            env,
            importer,
            session,
            acquisition,
...
def import_bids_meg_ctf_head_shape_file(
    env: Env,
    importer: BidsImporter,
    session: DbSession,
    data_type: str,
    ...
    """
    Import the CTF head shape file shared by the acquisitions of a BIDS MEG data type.
    """
    ...
    loris_file_path = get_loris_bids_file_path(
        ...
        session,
        data_type,
        head_shape_file.path,
    )

    db_head_shape_file = try_get_meg_ctf_head_shape_file_with_path(env.db, loris_file_path)
    if db_head_shape_file is not None:
        return db_head_shape_file

    copy_loris_bids_file(importer, head_shape_file.path, loris_file_path)
    bids_file = get_or_create_loris_bids_file(env, importer, head_shape_file.path, loris_file_path)
    return insert_meg_ctf_head_shape_file(env, head_shape_file, loris_file_path, bids_file)


def import_bids_meg_ctf_acquisition(
    env: Env,
    importer: BidsImporter,
    session: DbSession,
    ...
    """
    Import a BIDS MEG CTF acquisition into LORIS.
    """
    ...
    modality, output_type, file_type, file_hash = group_errors_tuple(
        ...
        lambda: get_check_bids_physio_output_type(env, importer.args.type or 'raw'),
