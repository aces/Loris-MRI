...
from loris_bids_utils.utils import get_pybids_file_path, try_get_pybids_value
...
class BidsMegDataTypeReader(BidsDataTypeReader):
    ...
    @cached_property
    def acquisitions(self) -> list[tuple[MegCtfAcquisition, BidsAcquisitionInfo]]:
        """
        The MEG acquisitions found in the MEG data type.
        """

        acquisitions: list[tuple[MegCtfAcquisition, BidsAcquisitionInfo]] = []
        for ctf_name in find_dir_meg_acquisition_names(self.path):
            ...
            acquisition = MegCtfAcquisition(self.path / ctf_name, self.head_shape_file)
    ...
    def head_shape_file(self) -> MegCtfHeadShapeFile | None:
        ...
        return MegCtfHeadShapeFile.read(get_pybids_file_path(head_shape_file))
