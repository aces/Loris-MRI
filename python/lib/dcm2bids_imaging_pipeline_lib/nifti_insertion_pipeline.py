...
class NiftiInsertionPipeline(BasePipeline):
    ...
    def __init__(self, loris_getopt_obj, script_name):
        ...
        self.nifti_blake2 = lib.utilities.compute_blake2b_hash(self.nifti_path)
        self.nifti_md5 = lib.utilities.compute_md5_hash(self.nifti_path)
        self.json_path = self.options_dict["json_path"]["value"]
        self.json_blake2 = lib.utilities.compute_blake2b_hash(self.json_path) if self.json_path else None
        self.bval_path = self.options_dict["bval_path"]["value"]
        self.bval_blake2 = lib.utilities.compute_blake2b_hash(self.bval_path) if self.bval_path else None
        self.bvec_path = self.options_dict["bvec_path"]["value"]
        self.bvec_blake2 = lib.utilities.compute_blake2b_hash(self.bvec_path) if self.bval_path else None
        self.json_md5 = lib.utilities.compute_md5_hash(self.json_path)
