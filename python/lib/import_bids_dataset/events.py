...
from loris_bids_reader.data_dictionary import BIDSDataDictFile, BIDSFieldDescription
...
def insert_events_metadata_file(
    ...
    events_metadata_file: BIDSDataDictFile,
):
    """
    ...
    event_file = DbPhysioEventFile(
        ...
        type           = 'json',
        path           = events_metadata_file.path,
    )
    ...
    for event_name, event in events_metadata_file.data.root.items():
    ...
    if source.physio_file is not None:
        ...
        blake2 = compute_file_blake2b_hash(events_metadata_file.path)
