...
def get_root_events_metadata(
    ...
    events_dict_file = bids.events_dict_file

    if events_dict_file is None:
    ...
    if loris_bids_path is not None:
        events_metadata_rel_path = events_dict_file.path.relative_to(loris_bids_path)
        events_metadata_path = loris_bids_path / events_metadata_rel_path
        shutil.copyfile(events_dict_file.path, events_metadata_path, args.verbose)  # type: ignore
    else:
        events_metadata_path = events_dict_file.path

    _, dataset_tag_dict = insert_events_metadata_file(env, DatasetSource(project_id), events_dict_file)
