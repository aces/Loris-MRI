...
from lib.db.queries.cohort import try_get_cohort_with_title
...
def check_or_create_bids_candidates_and_sessions(
    ...
    for bids_participant in bids_reader.bids_participants:
        ...
        if bids_participant.cohort is not None:
            cohort = try_get_cohort_with_title(env.db, bids_participant.cohort)

        bids_sessions = bids_reader.cand_sessions_list[bids_participant.id]

        if bids_sessions == []:
            default_visit_label = cast(str, get_config_with_setting_name(env.db, 'default_bids_vl').value)
