...
def check_or_create_bids_subject(env: Env, tsv_participant: BIDSParticipantRow, create_candidate: bool) -> DbCandidate:
    ...
    try:
        ...
        if candidate is None:
            raise CheckBidsSubjectSessionError(
                f"No LORIS candidate found for the BIDS participant ID '{tsv_participant.participant_id}' (identified as a CandID)."
            )
    ...
    if not create_candidate:
        raise CheckBidsSubjectSessionError(
            f"No LORIS candidate found for the BIDS participant ID '{tsv_participant.participant_id}' (identified as a PSCID)."
        )
