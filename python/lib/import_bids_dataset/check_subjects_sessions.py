...
def check_or_create_bids_subject_and_sessions(
    ...
    participant = subject.root_dataset.tsv_participants and subject.root_dataset.tsv_participants.get_row(subject.label)
    ...
    candidate = check_or_create_bids_subject(env, participant, create_candidate)

    if create_session:
        cohort = get_tsv_participant_cohort(env, participant)
    else:
        cohort = None
...
def check_or_create_bids_subject(env: Env, participant: BidsParticipantTsvRow, create_candidate: bool) -> DbCandidate:
    """
    ...
    try:
        cand_id = int(participant.participant_id)
        candidate = try_get_candidate_with_cand_id(env.db, cand_id)
        if candidate is None:
            raise CheckBidsSubjectSessionError(
                f"No LORIS candidate found for the BIDS participant ID '{participant.participant_id}' (identified"
                " as a CandID)."
            )
    ...
    candidate = try_get_candidate_with_psc_id(env.db, participant.participant_id)
    if candidate is not None:
        return candidate

    if not create_candidate:
        raise CheckBidsSubjectSessionError(
            f"No LORIS candidate found for the BIDS participant ID '{participant.participant_id}' (identified as a"
            " PSCID)."
        )

    return create_bids_candidate(env, participant)
...
def create_bids_candidate(env: Env, participant: BidsParticipantTsvRow) -> DbCandidate:
    ...
    birth_date = get_tsv_participant_birth_date(participant)

    sex = get_tsv_participant_sex(env, participant)

    site = get_tsv_participant_site(env, participant)

    project = get_tsv_participant_project(env, participant)
...
def get_tsv_participant_birth_date(participant: BidsParticipantTsvRow) -> datetime | None:
    """
    ...
    try:
        return parse(participant.birth_date)
    except ParserError:
        raise CheckBidsSubjectSessionError(
            f"Could not parse the BIDS participants.tsv birth date '{participant.birth_date}'."
        )


def get_tsv_participant_sex(env: Env, participant: BidsParticipantTsvRow) -> str | None:
    """
    ...
    if participant.data['sex'] is None:
        return None
...
def get_tsv_participant_site(env: Env, participant: BidsParticipantTsvRow) -> DbSite:
    """
    ...
    if participant.data['site'] is None:
        raise CheckBidsSubjectSessionError(
...
def get_tsv_participant_project(env: Env, participant: BidsParticipantTsvRow) -> DbProject:
    """
    ...
    if participant.data['project'] is None:
        raise CheckBidsSubjectSessionError(
...
def get_tsv_participant_cohort(env: Env, participant: BidsParticipantTsvRow) -> DbCohort:
    """
    ...
    if participant.data['cohort'] is None:
        raise CheckBidsSubjectSessionError(
            "No 'cohort' column found in the BIDS participants.tsv file, this field is required to create session."
        )

    cohort = try_get_cohort_with_name(env.db, participant.data['cohort'])
    if cohort is None:
        raise CheckBidsSubjectSessionError(
            f"No cohort found for the BIDS participants.tsv cohort name '{participant.data['cohort']}'."
        )
