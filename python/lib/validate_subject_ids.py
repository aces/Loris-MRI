...
def validate_subject_parts(
    ...
    return validate_subject(db, verbose, subject, create_visit)
...
def validation_error_name(subject_name: str, message: str):
    raise ValidateSubjectException(f'Validation error for subject \'{subject_name}\'.\n{message}')


def validation_error_subject(subject: Subject, message: str):
    return validation_error_name(subject.get_name(), message)
