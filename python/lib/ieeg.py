"""Deals with iEEG BIDS datasets and register them into the database."""
...
class Ieeg:
        ...
        for row in bids_reader.cand_session_modalities_list:
            for modality in row['modalities']:
                if modality == 'ieeg':
                    ...
                    loris_bids_ieeg_rel_dir = "sub-" + row['bids_sub_id'] + "/" + \
                                             "ses-" + visit_label + "/ieeg/"
                    lib.utilities.create_dir(
                        loris_bids_root_dir + loris_bids_ieeg_rel_dir, verbose
                    )
                    Ieeg(
                        ...
                        loris_bids_ieeg_rel_dir = loris_bids_ieeg_rel_dir,
    ...
    def __init__(self, bids_reader, bids_sub_id, bids_ses_id, bids_modality, db,
                 ...
                 loris_bids_ieeg_rel_dir, loris_bids_root_dir):
        """
        Constructor method for the ieeg class.
        ...
        :param bids_modality: BIDS modality (a.k.a. iEEG)
        ...
        :param loris_bids_ieeg_rel_dir: LORIS BIDS EEG relative dir path to data_dir
         :type loris_bids_ieeg_rel_dir: str
        ...
        # load the LORIS BIDS import root directory where the ieeg files will
        # be copied
        self.loris_bids_ieeg_rel_dir = loris_bids_ieeg_rel_dir
        ...
        #     self.derivative_ieeg_files = self.grep_bids_derivatives_ieeg_files()
    ...
    def grep_bids_derivatives_ieeg_files(self):
        """
        ...
        exclude_types = ['channels', 'electrodes', 'ieeg', 'events']
    ...
    def register_raw_data(self):
        ...
        # insert EEG file
        inserted_ieeg  = self.fetch_and_insert_ieeg_file()
        ieeg_file_id   = inserted_ieeg['file_id']
        ieeg_file_path = inserted_ieeg['ieeg_path']

        # insert related electrode, channel and event information
        electrode_file_path = self.fetch_and_insert_electrode_file(ieeg_file_id)
        channel_file_path   = self.fetch_and_insert_channel_file(ieeg_file_id)
        event_file_path     = self.fetch_and_insert_event_file(ieeg_file_id)
        ...
        archive_rel_name = os.path.splitext(ieeg_file_path)[0] + ".tgz"
        self.create_and_insert_archive(
            files_to_archive, archive_rel_name, ieeg_file_id
        )

        # create data chunks for React visualization in
        # data_dir/bids_import/bids_dataset_name_BIDSVersion_chunks directory
        physiological.create_chunks_for_visualization(ieeg_file_id, self.data_dir)
    ...
    def register_derivatives_data(self):
        ...
        for derivatives in self.bids_reader.derivatives_list:
            if derivatives['parent']:
                ...
                ieeg_file_id = self.fetch_and_insert_ieeg_file(derivatives)

                # insert related electrode, channel and event information
                if ieeg_file_id:
                    self.fetch_and_insert_electrode_file(ieeg_file_id, derivatives)
                    self.fetch_and_insert_channel_file(ieeg_file_id, derivatives)
                    self.fetch_and_insert_event_file(ieeg_file_id, derivatives)

    def fetch_and_insert_ieeg_file(self, derivatives=None):
        """
        ...
        # grep for the ieeg file
        derivative_pattern = None
        derivative_path    = None
        files_list         = self.ieeg_files  # by default, raw data is ieeg_files
        if derivatives:
            # TODO grep the source file as well as the input file ID???
            derivative_pattern = derivatives['derivative_name'] + "/sub-"
            files_list         = self.derivative_ieeg_files
            derivative_path    = self.get_derivatives_path(derivatives)

        # grep the raw files from ieeg_files list
        ieeg_file = BidsReader.grep_file(
            files_list         = files_list,
            match_pattern      = ".(set$|edf$|vhdr$|vmrk$|ieeg$|bdf$)",
        ...
        # return if no ieeg_file was found
        if not ieeg_file:
            return None
        ...
        ieeg_acq_time = None
        if self.scans_file:
            scan_info = ScansTSV(self.scans_file, ieeg_file, self.verbose)
            ieeg_acq_time = scan_info.get_acquisition_time()
            ieeg_file_data['age_at_scan'] = scan_info.get_age_at_scan()
            ...
            ieeg_file_data['scans_tsv_file'] = scans_path
            scans_blake2 = blake2b(self.scans_file.encode('utf-8')).hexdigest()
            ieeg_file_data['physiological_scans_tsv_file_bake2hash'] = scans_blake2

        # if file type is set and fdt file exists, append fdt path to the
        # ieeg_file_data dictionary
        if file_type == 'set' and fdt_file:
            ...
            ieeg_file_data['fdt_file'] = fdt_path
            fdt_blake2 = blake2b(fdt_file.encode('utf-8')).hexdigest()
            ieeg_file_data['physiological_fdt_file_blake2b_hash'] = fdt_blake2

        # append the blake2b to the ieeg_file_data dictionary
        blake2 = blake2b(ieeg_file.encode('utf-8')).hexdigest()
        ieeg_file_data['physiological_file_blake2b_hash'] = blake2
        ...
        ieeg_path       = result['FilePath']            if result else None
        if not physio_file_id:
            ...
            # copy the ieeg_file to the LORIS BIDS import directory
            ieeg_path = self.copy_file_to_loris_bids_dir(
                ieeg_file, derivative_path
            )
            ...
            physio_file_id = physiological.insert_physiological_file(
                ieeg_file_info, ieeg_file_data
            )

        # if the iEEG file was a set file, then update the filename for the .set
        # and .fdt files in the .set file so it can find the proper file for
        # visualization and analyses
        if file_type == 'set':
            set_full_path = self.data_dir + ieeg_path
            fdt_full_path = ieeg_file_data['fdt_file']
            if fdt_full_path:
                fdt_full_path = self.data_dir + ieeg_file_data['fdt_file']
            utilities.update_set_file_path_info(set_full_path, fdt_full_path)

        return {'file_id': physio_file_id, 'ieeg_path': ieeg_path }
    ...
    def copy_file_to_loris_bids_dir(self, file, derivatives_path=None):
        ...
        copy_file = self.loris_bids_ieeg_rel_dir
        if self.bids_ses_id:
        ...
        if derivatives_path:
            ...
            lib.utilities.create_dir(
                derivatives_path + self.loris_bids_ieeg_rel_dir,
                self.verbose
            )
    ...
    def create_and_insert_archive(self, files_to_archive, archive_rel_name,
        ...
        :param ieeg_file_id     : PhysiologicalFileID
         :type ieeg_file_id     : int
        """
        ...
        result = physiological.grep_archive_info_from_file_id(ieeg_file_id)
        if result:
            if not blake2:
                message = '\nERROR: no archive was found on the filesystem ' + \
                          'while an entry was found in the database for '   + \
                          'PhysiologicalFileID = ' + str(ieeg_file_id)
                print(message)
        ...
        blake2 = blake2b(archive_full_path.encode('utf-8')).hexdigest()
        archive_info = {
            'PhysiologicalFileID': ieeg_file_id,
            'Blake2bHash'        : blake2,
