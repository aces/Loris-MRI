...
from lib.db.queries.physio import try_get_physio_file_with_path
...
def test_import_eeg_bids_dataset():
    ...
    # Check that the physiological file had been inserted in the database.
    physio_file = try_get_physio_file_with_path(
        db,
        'bids_imports/Face13_BIDSVersion_1.1.0/sub-OTT166/ses-V1/eeg/sub-OTT166_ses-V1_task-faceO_eeg.edf',
    )
    assert physio_file is not None

    # Check that the physiological file parameters had been inserted in the database.
    parameters = get_physio_file_parameters_dict(db, physio_file.id)
    assert parameters == {
